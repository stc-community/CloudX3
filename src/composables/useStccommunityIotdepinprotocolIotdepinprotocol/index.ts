...
export default function useStccommunityIotdepinprotocolIotdepinprotocol() {
  ...
  const QueryParams = (options: any) => {
    ...
    return useQuery(
      [key],
      () => {
        return client.StccommunityIotdepinprotocolIotdepinprotocol.query
          .queryParams()
          .then(res => res.data);
      },
      options
  ...
  const QueryDevice = (address: string, query: any, options: any) => {
    ...
    return useQuery(
      ...
      () => {
        ...
        return client.StccommunityIotdepinprotocolIotdepinprotocol.query
          .queryDevice(address, query ?? undefined)
          .then(res => res.data);
      },
      options
    );
  ...
  const QueryDeviceAll = (query: any, options: any, perPage: number) => {
    ...
    return useInfiniteQuery(
      ...
      ({ pageParam = 1 }: { pageParam?: number }) => {
        ...
        query["pagination.count_total"] = true;
        return client.StccommunityIotdepinprotocolIotdepinprotocol.query
          .queryDeviceAll(query ?? undefined)
          .then(res => ({ ...res.data, pageParam }));
      },
      {
        ...options,
  ...
  const QueryEventPbAll = (query: any, options: any, perPage: number) => {
    ...
    return useInfiniteQuery(
      ...
      ({ pageParam = 1 }: { pageParam?: number }) => {
        ...
        query["pagination.count_total"] = true;
        return client.StccommunityIotdepinprotocolIotdepinprotocol.query
          .queryEventPbAll(query ?? undefined)
          .then(res => ({ ...res.data, pageParam }));
      },
      {
        ...options,
